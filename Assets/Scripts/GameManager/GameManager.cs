...
public class GameManager : MonoBehaviour
{
    ...
    protected void OnEnable()
    {
        ...
        {
            Debug.LogError("ConsumableDatabase is not assigned in GameManager!");
        }
        ...
        {
            Debug.LogWarning("No states assigned to GameManager!");
            return;
        }
        ...
        for (int i = 0; i < states.Length; ++i)
        {
            if (states[i] != null)
            {
                ...
                {
                    m_StateDict.Add(stateName, states[i]);
                }
                else
                {
                    Debug.LogWarning($"Duplicate state name found: {stateName}");
                }
            }
            else
            {
                Debug.LogWarning($"State at index {i} is null!");
            }
        }
    ...
    {
        if (m_StateStack.Count > 0 && m_StateStack[m_StateStack.Count - 1] != null)
        {
